...
const style = '';
const commonjsGlobal$1 = typeof globalThis !== 'undefined' ? globalThis : typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : typeof self !== 'undefined' ? self : {};
const sweetalert2 = { exports: {} };
/*!
* sweetalert2 v11.7.1
...
(function (module, exports) {
  (function (global2, factory) {
    ...
    const elementByClass = (className) => elementBySelector(`.${className}`);
...
`;
    const getFocusableElements = () => {
      const focusableElementsWithTabindex = Array.from(getPopup().querySelectorAll('[tabindex]:not([tabindex="-1"]):not([tabindex="0"])')).sort((a, b2) => {
    ...
    const isLoading = () => getPopup().hasAttribute('data-loading');
    const states = {
    ...
    const show = function (elem) {
      const display = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'flex';
    ...
    const toggle = function (elem, condition) {
      const display = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 'flex';
    ...
    const animateTimerProgressBar = function (timer) {
      const reset = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
...
`.replace(/(^|\n)\s*/g, '');
    ...
    const getInputContainer = (inputType) => getDirectChildByClass(getPopup(), swalClasses[inputType] || swalClasses.input);
...
`;
    ...
    const isVisible = () => isVisible$1(getPopup());
    ...
    const isStylus = (event) => event.touches && event.touches.length && event.touches[0].touchType === 'stylus';
    const isZoom = (event) => event.touches && event.touches.length > 1;
    const undoIOSfix = () => {
    ...
    const isValidParameter = (paramName) => Object.prototype.hasOwnProperty.call(defaultParams, paramName);
    const isUpdatableParameter = (paramName) => updatableParams.indexOf(paramName) !== -1;
    const isDeprecatedParameter = (paramName) => deprecatedParams[paramName];
    ...
    function update(params) {
      ...
      const updatedParams = { ...innerParams, ...validUpdatableParams };
      render(this, updatedParams);
      privateProps.innerParams.set(this, updatedParams);
      Object.defineProperties(this, {
        params: {
          value: { ...this.params, ...params },
          writable: false,
          enumerable: true,
        },
      });
    ...
    const isSelected = (optionValue, inputValue) => inputValue && inputValue.toString() === optionValue.toString();
    ...
    const isAnyButtonShown = (innerParams) => innerParams.showConfirmButton || innerParams.showDenyButton || innerParams.showCancelButton || innerParams.showCloseButton;
    ...
    function mixin(mixinParams) {
      class MixinSwal extends this {
        _main(params, priorityMixinParams) {
          return super._main(params, { ...mixinParams, ...priorityMixinParams });
        }
      }
      return MixinSwal;
    }
    const getTimerLeft = () => globalState.timeout && globalState.timeout.getTimerLeft();
    const stopTimer = () => {
    ...
    const isTimerRunning = () => globalState.timeout && globalState.timeout.isRunning();
    let bodyClickListenerAdded = false;
    const clickHandlers = {};
    function bindClickHandler() {
      const attr = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 'data-swal-template';
    ...
    class SweetAlert {
      ...
      _main(userParams) {
        const mixinParams = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
        showWarningsForParams({ ...mixinParams, ...userParams });
    ...
    const prepareParams = (userParams, mixinParams) => {
      ...
      const params = {
        ...defaultParams, ...mixinParams, ...templateParams, ...userParams,
      };
      params.showClass = { ...defaultParams.showClass, ...params.showClass };
      params.hideClass = { ...defaultParams.hideClass, ...params.hideClass };
      return params;
    };
...
const Swal = sweetalert2.exports;
/**
 * @license
...
function _errorWithCustomMessage(auth2, code, message) {
  const errorMap = { ...prodErrorMap(), [code]: message };
...
function _addTidIfNecessary(auth2, request) {
  if (auth2.tenantId && !request.tenantId) {
    return { ...request, tenantId: auth2.tenantId };
  }
  return request;
}
async function _performApiRequest(auth2, method, path, request, customErrorMap = {}) {
  return _performFetchWithErrorHandling(auth2, customErrorMap, async () => {
    ...
    const query = querystring({ key: auth2.config.apiKey, ...params }).slice(1);
    const headers = await auth2._getAdditionalHeaders();
    headers['Content-Type'] = 'application/json';
    if (auth2.languageCode) {
      headers['X-Firebase-Locale'] = auth2.languageCode;
    }
    return FetchProvider.fetch()(_getFinalTarget(auth2, auth2.config.apiHost, path, query), {
      method,
      headers,
...
function extractProviderData(providers) {
  return providers.map((_a2) => {
    const { providerId } = _a2; const
      provider2 = __rest(_a2, ['providerId']);
    return {
      providerId,
      uid: provider2.rawId || '',
...
class UserImpl {
  constructor(_a2) {
    const { uid, auth: auth2, stsTokenManager } = _a2; const
      opt = __rest(_a2, ['uid', 'auth', 'stsTokenManager']);
  ...
  _clone(auth2) {
    return new UserImpl({ ...this, auth: auth2, stsTokenManager: this.stsTokenManager._clone() });
  }
  ...
  static _fromJSON(auth2, object) {
    let _a2; let _b; let _c2; let _d; let _e2; let _f; let _g; let
      _h;
...
function _getBrowserName(userAgent) {
  ...
  if (ua2.includes("opera/") || ua2.includes("opr/") || ua2.includes("opios/")) {
    ...
    return "IEMobile";
  } if (ua2.includes("msie") || ua2.includes("trident/")) {
    return "IE";
  } if (ua2.includes("edge/")) {
    return "Edge";
  } if (_isFirefox(ua2)) {
    return "Firefox";
  } if (ua2.includes("silk/")) {
    return "Silk";
  } if (_isBlackBerry(ua2)) {
    return "Blackberry";
  } if (_isWebOS(ua2)) {
    return "Webos";
  } if (_isSafari(ua2)) {
    return "Safari";
  } if ((ua2.includes("chrome/") || _isChromeIOS(ua2)) && !ua2.includes("edge/")) {
    return "Chrome";
  } if (_isAndroid(ua2)) {
    return "Android";
  } 
...
class OAuthCredential extends AuthCredential {
  ...
  static fromJSON(json) {
    const obj = typeof json === 'string' ? JSON.parse(json) : json;
    const { providerId, signInMethod } = obj; const
...
class ActionCodeURL {
  constructor(actionLink) {
    let _a2; let _b; let _c2; let _d; let _e2; let
      _f;
...
class AbstractPopupRedirectOperation {
  ...
  async onAuthEvent(event) {
    const {
      urlResponse, sessionId, postBody, tenantId, error, type,
    } = event;
    if (error) {
      this.reject(error);
      return;
...
var da = `closure_uid_${1e9 * Math.random() >>> 0}`; var
  ea$1 = 0;
...
function q$1(a, b2, c) {
  Function.prototype.bind && Function.prototype.bind.toString().indexOf('native code') != -1 ? q$1 = fa$1 : q$1 = ha$1;
...
const la$1 = Array.prototype.indexOf ? function (a, b2) {
  ...
  if (typeof a === 'string') { return typeof b2 !== 'string' || b2.length != 1 ? -1 : a.indexOf(b2, 0); }
...
const wa$1 = x$1('Opera'); const y = x$1('Trident') || x$1('MSIE'); const xa = x$1('Edge'); const ya$1 = xa || y; const za$1 = x$1('Gecko') && !(sa$1().toLowerCase().indexOf('webkit') != -1 && !x$1('Edge')) && !(x$1('Trident') || x$1('MSIE')) && !x$1('Edge'); const
  Aa$1 = sa$1().toLowerCase().indexOf('webkit') != -1 && !x$1('Edge');
function Ba() {
  const a = l.document;
...
function Ia$1() {
  return ua$1(() => {
    let a = 0;
    const b2 = qa(String(Ea)).split('.'); const c = qa('9').split('.'); const
      d = Math.max(b2.length, c.length);
...
function z$1(a, b2) {
  ...
  this.i = null;
  if (a) {
    const c = this.type = a.type; const
    ...
    } else c == 'mouseover' ? b2 = a.fromElement : c == 'mouseout' && (b2 = a.toElement);
    ...
    this.pointerType = typeof a.pointerType === 'string' ? a.pointerType : Ma$1[a.pointerType] || '';
    this.state = a.state;
    this.i = a;
...
const A = `closure_listenable_${1e6 * Math.random() | 0}`;
let Na = 0;
...
Ua$1.prototype.add = function (a, b2, c, d, e) {
  ...
  a || (a = this.g[f] = [], this.h++);
  const h = Va(a, b2, d, e);
  h > -1 ? (b2 = a[h], c || (b2.ea = false)) : (b2 = new Oa$1(b2, this.src, f, !!d, e), b2.ea = c, a.push(b2));
  return b2;
};
function Wa(a, b2) {
  const c = b2.type;
  if (c in a.g) {
    const d = a.g[c]; const e = la$1(d, b2); let
      f;
...
const Xa = `closure_lm_${1e6 * Math.random() | 0}`; const
  Ya = {};
function $a$1(a, b2, c, d, e) {
  if (d && d.once) { return ab(a, b2, c, d, e); }
  if (Array.isArray(b2)) {
    for (let f = 0; f < b2.length; f++) { $a$1(a, b2[f], c, d, e); }
    return null;
  }
  c = bb(c);
...
function cb(a, b2, c, d, e, f) {
  ...
  d.src = a;
  d.listener = c;
  if (a.addEventListener) { oa$1 || (e = h), void 0 === e && (e = false), a.addEventListener(b2.toString(), d, e); } else if (a.attachEvent) { a.attachEvent(fb(b2.toString()), d); } else if (a.addListener && a.removeListener) { a.addListener(d); } else { throw Error('addEventListener and attachEvent are unavailable.'); }
  return c;
}
function eb() {
  function a(c) {
...
function ab(a, b2, c, d, e) {
  if (Array.isArray(b2)) {
    for (let f = 0; f < b2.length; f++) { ab(a, b2[f], c, d, e); }
    return null;
  }
  c = bb(c);
...
function hb(a, b2, c, d, e) {
  if (Array.isArray(b2)) { for (var f = 0; f < b2.length; f++) hb(a, b2[f], c, d, e); } else { (d = p(d) ? !!d.capture : !!d, c = bb(c), a && a[A]) ? (a = a.i, b2 = String(b2).toString(), b2 in a.g && (f = a.g[b2], c = Va(f, c, d, e), c > -1 && (Pa(f[c]), Array.prototype.splice.call(f, c, 1), f.length == 0 && (delete a.g[b2], a.h--)))) : a && (a = db$1(a)) && (b2 = a.g[b2.toString()], a = -1, b2 && (a = Va(b2, c, d, e)), (c = a > -1 ? b2[a] : null) && ib(c)); }
}
function ib(a) {
  if (typeof a !== 'number' && a && !a.ba) {
    const b2 = a.src;
    if (b2 && b2[A]) { Wa(b2.i, a); } else {
      let c = a.type; const
        d = a.proxy;
      ...
      (c = db$1(b2)) ? (Wa(c, a), c.h == 0 && (c.src = null, b2[Xa] = null)) : Pa(a);
    }
  }
}
function fb(a) {
...
const jb = `__closure_events_fn_${1e9 * Math.random() >>> 0}`;
function bb(a) {
...
function C$1(a, b2) {
  ...
  a = a.P;
  d = b2.type || b2;
  if (typeof b2 === 'string') { b2 = new w(b2, a); } else if (b2 instanceof w) { b2.target = b2.target || a; } else {
    var e = b2;
    b2 = new w(d, a);
    Ta$1(b2, e);
  }
...
B$1.prototype.M = function () {
  ...
  if (this.i) {
    const a = this.i; let
      c;
    for (c in a.g) {
      for (let d = a.g[c], e = 0; e < d.length; e++) { Pa(d[e]); }
      delete a.g[c];
      a.h--;
    }
  }
...
k$1.lb = function () {
  if (this.ca) {
    const a = Date.now() - this.l;
    a > 0 && a < 0.8 * this.h ? this.R = this.g.setTimeout(this.j, this.h - a) : (this.R && (this.g.clearTimeout(this.R), this.R = null), C$1(this, 'tick'), this.ca && (zb(this), this.start()));
  }
};
k$1.start = function () {
  this.ca = true;
...
function Ab(a, b2, c) {
  if (typeof a === 'function') { c && (a = q$1(a, c)); } else if (a && typeof a.handleEvent === 'function') { a = q$1(a.handleEvent, a); } else { throw Error('Invalid listener argument'); }
...
function Eb(a, b2, c, d) {
  ...
  for (let e = 0; e < c.length; e++) {
    const f = $a$1(b2, c[e], d || a.handleEvent, false, a.h || a);
    if (!f) { break; }
    a.g[f.key] = f;
  }
}
...
function Hb(a, b2, c, d, e, f) {
  a.info(() => {
    if (a.g) {
      if (f) {
        var h = '';
        for (let n = f.split('&'), u = 0; u < n.length; u++) {
          let m = n[u].split('=');
          if (m.length > 1) {
            const r = m[0];
            m = m[1];
            const F = r.split('_');
            h = F.length >= 2 && F[1] == 'type' ? `${h}${r}=${m}&` : `${h}${r}=redacted&`;
          }
        }
      } else h = null;
    } else { h = f; }
...
function Ib(a, b2, c, d, e, f, h) {
  a.info(() => `XMLHTTP RESP (${d}) [ attempt ${e}]: ${b2}\n${c}\n${f} ${h}`);
}
function E(a, b2, c, d) {
  a.info(() => `XMLHTTP TEXT (${b2}): ${Jb(a, c)}${d ? ` ${d}` : ''}`);
}
function Kb(a, b2) {
  a.info(() => `TIMEOUT: ${b2}`);
}
        ...
        if (Array.isArray(c[a])) {
          ...
          if (!(d.length < 2)) {
            ...
            if (Array.isArray(e) && !(e.length < 1)) {
              const f = e[0];
              if (f != 'noop' && f != 'stop' && f != 'close') for (let h = 1; h < e.length; h++) e[h] = '';
            }
          }
        }
      }
...
const Qb = {
  NO_ERROR: 0, mb: 1, zb: 2, yb: 3, tb: 4, xb: 5, Ab: 6, Ma: 7, TIMEOUT: 8, Db: 9,
};
const Rb = {
  rb: 'complete', Nb: 'success', Na: 'error', Ma: 'abort', Fb: 'ready', Gb: 'readystatechange', TIMEOUT: 'timeout', Bb: 'incrementaldata', Eb: 'progress', ub: 'downloadprogress', Vb: 'uploadprogress',
};
function Sb() {
}
Sb.prototype.h = null;
...
function ec$1(a, b2) {
  ...
  a.N > 0 && (a.L = new Cb(q$1(a.La, a, a.g), a.N));
  ...
  try {
    if (a == this.g) {
      a: {
        ...
        let b2 = this.g.Ea();
        const F = this.g.aa();
        if (!(r < 3) && (r != 3 || ya$1 || this.g && (this.h.h || this.g.fa() || hc$1(this.g)))) {
          this.I || r != 4 || b2 == 7 || (b2 == 8 || F <= 0 ? H$1(3) : H$1(2));
          ic$1(this);
          let c = this.g.aa();
          this.Y = c;
          b:
          ...
          if (this.i) {
            ...
            this.P ? (lc$1(this, r, h), ya$1 && this.i && r == 3 && (Eb(this.S, this.T, 'tick', this.hb), this.T.start())) : (E(this.j, this.m, h, null), kc(this, h));
            r == 4 && P$1(this);
            this.i && !this.I && (r == 4 ? mc(this.l, this) : (this.i = false, N$1(this)));
          } else c == 400 && h.indexOf('Unknown SID') > 0 ? (this.o = 3, I(12)) : (this.o = 0, I(13)), P$1(this), Q$1(this);
        }
      }
    }
  } catch (r) {
...
function lc$1(a, b2, c) {
  ...
  a.i = a.i && d;
  d ? c.length > 0 && !a.$ && (a.$ = true, b2 = a.l, b2.g == a && b2.$ && !b2.K && (b2.j.info(`Great, no buffering proxy detected. Bytes received: ${c.length}`), oc$1(b2), b2.K = true, I(11))) : (E(
    a.j,
    a.m,
    c,
...
k$1.hb = function () {
  if (this.g) {
    const a = O$1(this.g); const
      b2 = this.g.fa();
    this.C < b2.length && (ic$1(this), lc$1(this, a, b2), this.i && a != 4 && N$1(this));
  }
};
function nc$1(a, b2) {
  let c = a.C; let
...
k$1.gb = function () {
  this.B = null;
  const a = Date.now();
  a - this.V >= 0 ? (Kb(this.j, this.A), this.K != 2 && (H$1(), I(17)), P$1(this), this.o = 2, Q$1(this)) : pc$1(this, this.V - a);
};
function Q$1(a) {
...
function kc(a, b2) {
  try {
    const c = a.l;
    if (c.G != 0 && (c.g == a || qc$1(c.h, a))) {
      if (!a.J && qc$1(c.h, a) && c.G == 3) {
        try {
        ...
        if (Array.isArray(d) && d.length == 3) {
          ...
          if (e[0] == 0) {
            a: {
              if (!c.u) {
                if (c.g) {
                  if (c.g.F + 3e3 < a.F) rc$1(c), sc$1(c);
                  else break a;
                }
                tc$1(c);
                I(18);
          ...
          } else { c.Ba = e[1], c.Ba - c.T > 0 && e[2] < 37500 && c.L && c.A == 0 && !c.v && (c.v = J(q$1(c.cb, c), 6e3)); }
          if (uc$1(c.h) <= 1 && c.ja) {
            try {
              c.ja();
            } catch (m) {
        ...
        for (e = c.Fa.g.parse(b2), b2 = 0; b2 < e.length; b2++) {
          ...
          if (c.G == 2) {
            if (m[0] == 'c') {
              ...
              const Ca = m[5];
              Ca != null && typeof Ca === 'number' && Ca > 0 && (d = 1.5 * Ca, c.J = d, c.j.info(`backChannelRequestTimeoutMs_=${d}`));
              d = c;
              const Z2 = a.g;
              if (Z2) {
                ...
                if (Da) {
                  const f = d.h;
                  f.g || Da.indexOf('spdy') == -1 && Da.indexOf('quic') == -1 && Da.indexOf('h2') == -1 || (f.j = f.l, f.g = /* @__PURE__ */ new Set(), f.h && (vc$1(f, f.h), f.h = null));
                }
                if (d.D) {
          ...
          } else c.G == 3 && (m[0] == 'stop' || m[0] == 'close' ? m[0] == 'stop' ? R(c, 7) : Ac$1(c) : m[0] != 'noop' && c.l && c.l.wa(m), c.A = 0);
        }
      }
    }
    H$1(4);
...
function Bc$1(a) {
  if (a.W && typeof a.W === 'function') { return a.W(); }
...
function Dc$1(a, b2) {
  if (a.forEach && typeof a.forEach === 'function') { a.forEach(b2, void 0); } else if (ba(a) || typeof a === 'string') { Array.prototype.forEach.call(a, b2, void 0); } else { for (let c = Cc$1(a), d = Bc$1(a), e = d.length, f = 0; f < e; f++) b2.call(void 0, d[f], c && c[f], a); }
}
const Ec$1 = RegExp('^(?:([^:/?#.]+):)?(?://(?:([^\\\\/?#]*)@)?([^\\\\/?#]*?)(?::([0-9]+))?(?=[\\\\/?#]|$))?([^?#]+)?(?:\\?([^#]*))?(?:#([\\s\\S]*))?$');
function Fc$1(a, b2) {
  if (a) {
    a = a.split('&');
    for (let c = 0; c < a.length; c++) {
      const d = a[c].indexOf('='); let
        e = null;
      if (d >= 0) {
...
function T(a, b2) {
  ...
  if (a instanceof T) {
    ...
    Jc(this, c);
    this.o = a.o;
  } else { a && (c = String(a).match(Ec$1)) ? (this.h = !!b2, Gc$1(this, c[1] || '', true), this.s = Kc$1(c[2] || ''), this.g = Kc$1(c[3] || '', true), Hc(this, c[4]), this.l = Kc$1(c[5] || '', true), Jc(this, c[6] || '', true), this.o = Kc$1(c[7] || '')) : (this.h = !!b2, this.i = new Ic$1(null, this.h)); }
}
T.prototype.toString = function () {
  ...
  let c = this.g;
  if (c || b2 == 'file') { a.push('//'), (b2 = this.s) && a.push(Lc$1(b2, Mc$1, true), '@'), a.push(encodeURIComponent(String(c)).replace(/%25([0-9a-fA-F]{2})/g, '%$1')), c = this.m, c != null && a.push(':', String(c)); }
  if (c = this.l) { this.g && c.charAt(0) != '/' && a.push('/'), a.push(Lc$1(c, c.charAt(0) == '/' ? Nc$1 : Oc$1, true)); }
...
function Lc$1(a, b2, c) {
  return typeof a === 'string' ? (a = encodeURI(a).replace(b2, Sc$1), c && (a = a.replace(/%25([0-9a-fA-F]{2})/g, '%$1')), a) : null;
}
function Sc$1(a) {
  a = a.charCodeAt(0);
  return `%${(a >> 4 & 15).toString(16)}${(a & 15).toString(16)}`;
}
var Mc$1 = /[#\/\?@]/g; var Oc$1 = /[#\?:]/g; var Nc$1 = /[#\?]/g; var Rc$1 = /[#\?@]/g; var
  Pc$1 = /#/g;
function Ic$1(a, b2) {
  this.h = this.g = null;
  this.i = a || null;
...
function U$1(a) {
  a.g || (a.g = /* @__PURE__ */ new Map(), a.h = 0, a.i && Fc$1(a.i, (b2, c) => {
...
k$1.oa = function () {
  U$1(this);
  const a = Array.from(this.g.values()); const b2 = Array.from(this.g.keys()); const
    c = [];
...
k$1.W = function (a) {
  U$1(this);
  let b2 = [];
  if (typeof a === 'string') { Uc$1(this, a) && (b2 = b2.concat(this.g.get(V$1(this, a)))); } else {
...
function fc$1(a, b2, c) {
  Tc$1(a, b2);
  c.length > 0 && (a.i = null, a.g.set(V$1(a, b2), ma$1(c)), a.h += c.length);
}
k$1.toString = function () {
  ...
  for (let c = 0; c < b2.length; c++) {
    let d = b2[c];
    const f = encodeURIComponent(String(d)); const
      h = this.W(d);
...
function Wc(a) {
  this.l = a || Xc;
  l.PerformanceNavigationTiming ? (a = l.performance.getEntriesByType('navigation'), a = a.length > 0 && (a[0].nextHopProtocol == 'hq' || a[0].nextHopProtocol == 'h2')) : a = !!(l.g && l.g.Ga && l.g.Ga() && l.g.Ga().$b);
  this.j = a ? this.l : 1;
  this.g = null;
...
Wc.prototype.cancel = function () {
  this.i = Zc(this);
  if (this.h) { this.h.cancel(), this.h = null; } else if (this.g && this.g.size !== 0) {
...
k$1.send = function (a) {
  ...
  this.g = true;
  const b2 = {
    headers: this.v, method: this.C, credentials: this.m, cache: void 0,
  };
  a && (b2.body = a);
...
k$1.abort = function () {
  ...
  this.readyState >= 1 && this.g && this.readyState != 4 && (this.g = false, id(this));
  this.readyState = gd;
};
k$1.Wa = function (a) {
  if (this.g && (this.l = a, this.h || (this.status = this.l.status, this.statusText = this.l.statusText, this.h = a.headers, this.readyState = 2, hd(this)), this.g && (this.readyState = 3, hd(this), this.g))) {
    if (this.responseType === 'arraybuffer') a.arrayBuffer().then(this.Ua.bind(this), this.ga.bind(this));
...
k$1.Ta = function (a) {
  if (this.g) {
    if (this.u && a.value) { this.response.push(a.value); } else if (!this.u) {
      let b2 = a.value ? a.value : new Uint8Array(0);
...
k$1.getAllResponseHeaders = function () {
  ...
  const a = []; const
    b2 = this.h.entries();
  for (let c = b2.next(); !c.done;) { c = c.value, a.push(`${c[0]}: ${c[1]}`), c = b2.next(); }
  return a.join('\r\n');
};
function hd(a) {
...
k$1.da = function (a, b2, c, d) {
  ...
  !(la$1(nd, b2) >= 0) || d || e || c.set('Content-Type', 'application/x-www-form-urlencoded;charset=utf-8');
  for (const [f, h] of c) { this.g.setRequestHeader(f, h); }
  this.J && (this.g.responseType = this.J);
  'withCredentials' in this.g && this.g.withCredentials !== this.L && (this.g.withCredentials = this.L);
  try {
    pd(this), this.B > 0 && ((this.K = qd(this.g)) ? (this.g.timeout = this.B, this.g.ontimeout = q$1(this.qa, this)) : this.A = Ab(this.qa, this.B, this)), this.v = true, this.g.send(a), this.v = false;
  } catch (f) {
    od(this, f);
  }
};
function qd(a) {
  return y && Ia$1() && typeof a.timeout === 'number' && void 0 !== a.ontimeout;
}
k$1.qa = function () {
  typeof goog !== 'undefined' && this.g && (this.j = `Timed out after ${this.B}ms, aborting`, this.m = 8, C$1(this, 'timeout'), this.abort(8));
};
function od(a, b2) {
  a.h = false;
...
function td(a) {
  if (a.h && typeof goog !== 'undefined' && (!a.C[1] || O$1(a) != 4 || a.aa() != 2)) {
    if (a.v && O$1(a) == 4) { Ab(a.Ha, 0, a); } else if (C$1(a, 'readystatechange'), O$1(a) == 4) {
      a.h = false;
      try {
        const n = a.aa();
        switch (n) {
        ...
        if (!(c = b2)) {
          let d;
          if (d = n === 0) {
            let e = String(a.H).match(Ec$1)[1] || null;
            if (!e && l.self && l.self.location) {
              const f = l.self.location.protocol;
...
k$1.aa = function () {
  try {
    return O$1(this) > 2 ? this.g.status : -1;
  } catch (a) {
    return -1;
  }
};
...
function vd(a, b2, c) {
  ...
  d || (c = ud(c), typeof a === 'string' ? c != null && encodeURIComponent(String(c)) : S$1(a, b2, c));
}
function wd(a, b2, c) {
...
function Bd(a, b2) {
  if (uc$1(a.h) >= a.h.j - (a.m ? 1 : 0)) { return false; }
  if (a.m) { return a.i = b2.D.concat(a.i), true; }
  if (a.G == 1 || a.G == 2 || a.C >= (a.Za ? 0 : a.$a)) { return false; }
...
k$1.Ja = function (a) {
  if (this.m) {
    ...
    } else this.G == 3 && (a ? Ed(this, a) : this.i.length == 0 || Yc(this.h) || Ed(this));
  }
};
function Ed(a, b2) {
  let c;
...
function Dd(a, b2, c) {
  ...
  let d = a.l ? q$1(a.l.Ra, a.l, a) : null;
  a: {
    const e = a.i;
    let f = -1;
    for (; ;) {
      const h = [`count=${c}`];
      f == -1 ? c > 0 ? (f = e[0].h, h.push(`ofs=${f}`)) : f = 0 : h.push(`ofs=${f}`);
      let n = true;
      for (let u = 0; u < c; u++) {
        let m = e[u].h;
        const r = e[u].g;
        m -= f;
        if (m < 0) { f = Math.max(0, e[u].h - 100), n = false; } else {
          try {
            bd(r, h, `req${m}_`);
          } catch (F) {
            d && d(r);
...
k$1.Ia = function () {
  this.u = null;
  Fd(this);
  if (this.$ && !(this.K || this.g == null || this.P <= 0)) {
    const a = 2 * this.P;
...
k$1.cb = function () {
  this.v != null && (this.v = null, sc$1(this), tc$1(this), I(19));
};
function rc$1(a) {
...
function mc(a, b2) {
  ...
  if (a.g == b2) {
    ...
    a.g = null;
    var d = 2;
  } else if (qc$1(a.h, b2)) { c = b2.D, xc$1(a.h, b2), d = 1; } else { return; }
  if (a.G != 0) {
    if (a.pa = b2.Y, b2.i) {
      if (d == 1) {
    ...
    } else if (e = b2.o, e == 3 || e == 0 && a.pa > 0 || !(d == 1 && Bd(a, b2) || d == 2 && tc$1(a))) {
...
function R(a, b2) {
  ...
  if (b2 == 2) {
    ...
    a.l && (c = null);
    const d = q$1(a.kb, a);
    c || (c = new T('//www.google.com/images/cleardot.gif'), l.location && l.location.protocol == 'http' || Gc$1(c, 'https'), dc$1(c));
    cd(c.toString(), d);
  } else { I(2); }
  a.G = 0;
  a.l && a.l.va(b2);
...
function Ad(a) {
  ...
  if (a.l) {
    const b2 = Zc(a.h);
    if (b2.length != 0 || a.i.length != 0) { na(a.la, b2), na(a.la, a.i), a.h.i.length = 0, ma$1(a.i), a.i.length = 0; }
    a.l.ua();
  }
}
function wc(a, b2, c) {
  let d = c instanceof T ? M$1(c) : new T(c, void 0);
  if (d.g != '') { b2 && (d.g = `${b2}.${d.g}`), Hc(d, d.m); } else {
    let e = l.location;
    d = e.protocol;
    b2 = b2 ? `${b2}.${e.hostname}` : e.hostname;
    e = +e.port;
    const f = new T(null, void 0);
    d && Gc$1(f, d);
    b2 && (f.g = b2);
    e && Hc(f, e);
    c && (f.l = c);
...
function Hd() {
  if (y && !(Number(La) >= 10)) { throw Error('Environmental error: no available transport.'); }
}
...
function X$1(a, b2) {
  ...
  (b2 = b2 && b2.httpSessionIdParam) && !pa$1(b2) && (this.g.D = b2, a = this.h, a !== null && b2 in a && (a = this.h, b2 in a && delete a[b2]));
  this.j = new Y$1(this);
}
t(X$1, B$1);
X$1.prototype.m = function () {
  ...
  const a = this.g; const b2 = this.l; const
    c = this.h || void 0;
  I(0);
  a.V = b2;
  a.ia = c || {};
...
const createWebChannelTransport = function () {
  return new Hd();
};
...
const Event = G$1;
const Stat = {
  sb: 0, vb: 1, wb: 2, Pb: 3, Ub: 4, Rb: 5, Sb: 6, Qb: 7, Ob: 8, Tb: 9, PROXY: 10, NOPROXY: 11, Mb: 12, Ib: 13, Jb: 14, Hb: 15, Kb: 16, Lb: 17, ob: 18, nb: 19, pb: 20,
};
const FetchXmlHttpFactory = ed;
const WebChannel = Ub;
const XhrIo = W$1;
...
class Q {
  ...
  getToken() {
    const t2 = this.i; const
      e = this.forceRefresh;
    return this.forceRefresh = false, this.auth ? this.auth.getToken(e).then((e2) => (this.i !== t2 ? (C('FirebaseAuthCredentialsProvider', 'getToken aborted due to token change.'), this.getToken()) : e2 ? (M(typeof e2.accessToken === 'string'), new U(e2.accessToken, this.currentUser)) : null)) : Promise.resolve(null);
  }
  ...
  u() {
    ...
    return M(t2 === null || typeof t2 === 'string'), new P(t2);
  }
}
class j {
  ...
  I() {
    return this.g ? this.g() : (M(!(typeof this.h !== 'object' || this.h === null || !this.h.auth || !this.h.auth.getAuthHeaderValueForFirstParty)), this.h.auth.getAuthHeaderValueForFirstParty([]));
  }
...
class H {
  ...
  start(t2, e) {
    const n = (t3) => {
      t3.error != null && C('FirebaseAppCheckTokenProvider', `Error getting App Check token; using placeholder token instead. Error: ${t3.error.message}`);
  ...
  getToken() {
    ...
    return this.forceRefresh = false, this.appCheck ? this.appCheck.getToken(t2).then((t3) => (t3 ? (M(typeof t3.token === 'string'), this.A = t3.token, new z(t3.token)) : null)) : Promise.resolve(null);
  }
 ...
 */
function Y(t2) {
  const e = typeof self !== 'undefined' && (self.crypto || self.msCrypto); const
    n = new Uint8Array(t2);
  if (e && typeof e.getRandomValues === 'function') { e.getRandomValues(n); } else { for (let e2 = 0; e2 < t2; e2++) n[e2] = Math.floor(256 * Math.random()); }
  return n;
}
/**
 * @license
...
class X {
  static R() {
    const t2 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'; const
      e = Math.floor(256 / t2.length) * t2.length;
    let n = '';
    for (; n.length < 20;) {
      const s = Y(40);
      for (let i = 0; i < s.length; ++i) { n.length < 20 && s[i] < e && (n += t2.charAt(s[i] % t2.length)); }
    }
    return n;
  }
}
...
class nt {
  constructor(t2, e) {
    if (this.seconds = t2, this.nanoseconds = e, e < 0) { throw new L(B.INVALID_ARGUMENT, `Timestamp nanoseconds out of range: ${e}`); }
    if (e >= 1e9) { throw new L(B.INVALID_ARGUMENT, `Timestamp nanoseconds out of range: ${e}`); }
    if (t2 < -62135596800) { throw new L(B.INVALID_ARGUMENT, `Timestamp seconds out of range: ${t2}`); }
    if (t2 >= 253402300800) { throw new L(B.INVALID_ARGUMENT, `Timestamp seconds out of range: ${t2}`); }
  }
  ...
  static fromMillis(t2) {
    const e = Math.floor(t2 / 1e3); const
      n = Math.floor(1e6 * (t2 - 1e3 * e));
    return new nt(e, n);
  }
...
class ut extends it {
  ...
  static fromServerFormat(t2) {
    ...
    let n = ''; let
      s = 0;
    const i = () => {
      if (n.length === 0) { throw new L(B.INVALID_ARGUMENT, `Invalid field path (${t2}). Paths must not be empty, begin with '.', end with '.', or contain '..'`); }
      e.push(n), n = '';
    };
    let r = false;
    for (; s < t2.length;) {
      const e2 = t2[s];
      if (e2 === '\\') {
        if (s + 1 === t2.length) { throw new L(B.INVALID_ARGUMENT, `Path has trailing escape character: ${t2}`); }
        const e3 = t2[s + 1];
        if (e3 !== '\\' && e3 !== '.' && e3 !== '`') { throw new L(B.INVALID_ARGUMENT, `Path has invalid escape sequence: ${t2}`); }
        n += e3, s += 2;
      } else e2 === '`' ? (r = !r, s++) : e2 !== '.' || r ? (n += e2, s++) : (i(), s++);
    }
    if (i(), r) { throw new L(B.INVALID_ARGUMENT, `Unterminated \` in path: ${t2}`); }
    return new ut(e);
  }
...
function mt(t2, e) {
  const n = t2.toTimestamp().seconds; const s = t2.toTimestamp().nanoseconds + 1; const
    i = st.fromTimestamp(s === 1e9 ? new nt(n + 1, 0) : new nt(n, s));
...
async function Et(t2) {
  if (t2.code !== B.FAILED_PRECONDITION || t2.message !== It) { throw t2; }
...
class At {
  ...
  static or(t2) {
    ...
    for (const n of t2) { e = e.next((t3) => (t3 ? At.resolve(t3) : n())); }
    return e;
  }
...
function Jt(t2) {
  let e; let
    n;
  return ((n = (((e = t2 == null ? void 0 : t2.mapValue) === null || void 0 === e ? void 0 : e.fields) || {}).__type__) === null || void 0 === n ? void 0 : n.stringValue) === 'server_timestamp';
}
function Yt(t2) {
  ...
  switch (n) {
    ...
    case 3:
      return (function (t3, e2) {
        if (typeof t3.timestampValue === 'string' && typeof e2.timestampValue === 'string' && t3.timestampValue.length === e2.timestampValue.length) return t3.timestampValue === e2.timestampValue;
        const n2 = Wt(t3.timestampValue); const
          s = Wt(e2.timestampValue);
    ...
    case 2:
      return (function (t3, e2) {
        if ('integerValue' in t3 && 'integerValue' in e2) return zt(t3.integerValue) === zt(e2.integerValue);
        if ('doubleValue' in t3 && 'doubleValue' in e2) {
          const n2 = zt(t3.doubleValue); const
            s = zt(e2.doubleValue);
    ...
    case 10:
      return (function (t3, e2) {
        const n2 = t3.mapValue.fields || {}; const
          s = e2.mapValue.fields || {};
        if ($t(n2) !== $t(s)) return false;
        for (const t4 in n2) if (n2.hasOwnProperty(t4) && (void 0 === s[t4] || !ne(n2[t4], s[t4]))) return false;
        return true;
      }(t2, e));
    default:
      return O();
...
function ie(t2, e) {
  ...
  switch (n) {
    ...
    case 2:
      return (function (t3, e2) {
        const n2 = zt(t3.integerValue || t3.doubleValue); const
          s2 = zt(e2.integerValue || e2.doubleValue);
    ...
    case 6:
      return (function (t3, e2) {
        const n2 = Ht(t3); const
          s2 = Ht(e2);
        return n2.compareTo(s2);
      }(t2.bytesValue, e.bytesValue));
    case 7:
      return (function (t3, e2) {
        const n2 = t3.split('/'); const
          s2 = e2.split('/');
    ...
    case 10:
      return (function (t3, e2) {
        if (t3 === Zt.mapValue && e2 === Zt.mapValue) return 0;
        if (t3 === Zt.mapValue) return 1;
        if (e2 === Zt.mapValue) return -1;
        const n2 = t3.fields || {}; const s2 = Object.keys(n2); const i = e2.fields || {}; const
          r = Object.keys(i);
        s2.sort(), r.sort();
...
function re(t2, e) {
  if (typeof t2 === 'string' && typeof e === 'string' && t2.length === e.length) { return Z(t2, e); }
  const n = Wt(t2); const s = Wt(e); const
...
function ue(t2) {
  return 'nullValue' in t2 ? 'null' : 'booleanValue' in t2 ? `${t2.booleanValue}` : 'integerValue' in t2 ? `${t2.integerValue}` : 'doubleValue' in t2 ? `${t2.doubleValue}` : 'timestampValue' in t2 ? (function (t3) {
    ...
    let e2 = '[';
    let n2 = true;
    for (const s of t3.values || []) n2 ? n2 = false : e2 += ',', e2 += ue(s);
    return `${e2}]`;
  }(t2.arrayValue)) : 'mapValue' in t2 ? (function (t3) {
    const e2 = Object.keys(t3.fields || {}).sort();
    let n2 = '{';
    let s = true;
    for (const i of e2) s ? s = false : n2 += ',', n2 += `${i}:${ue(t3.fields[i])}`;
    return `${n2}}`;
  }(t2.mapValue)) : O();
  let e; let
    n;
...
function _e(t2) {
  if (t2.geoPointValue) {
    return {
      geoPointValue: { ...t2.geoPointValue },
    };
  }
  if (t2.timestampValue && typeof t2.timestampValue === 'object') {
    return {
      timestampValue: { ...t2.timestampValue },
    };
  }
  if (t2.mapValue) {
    const e = {
  ...
  if (t2.arrayValue) {
    ...
    for (let n = 0; n < (t2.arrayValue.values || []).length; ++n) { e.arrayValue.values[n] = _e(t2.arrayValue.values[n]); }
    return e;
  }
  return { ...t2 };
}
...
function Se(t2) {
  for (const e of t2.filters) { if (e instanceof be) return false; }
  return true;
}
function De(t2) {
  if (t2 instanceof Re) { return t2.field.canonicalString() + t2.op.toString() + oe(t2.value); }
  {
...
function Fe(t2, e) {
  let n;
  return (((n = e.arrayValue) === null || void 0 === n ? void 0 : n.values) || []).map((t3) => ct.fromName(t3.referenceValue));
}
class $e extends Re {
  constructor(t2, e) {
...
class Qe {
  constructor(t2, e, n, s) {
    ...
    let i = 1;
    for (; !t2.isEmpty();) {
      if (i = e ? n(t2.key, e) : 1, e && s && (i *= -1), i < 0) t2 = this.isReverse ? t2.left : t2.right;
      else {
        if (i === 0) {
  ...
  getNext() {
    ...
    if (this.isReverse) { for (t2 = t2.left; !t2.isEmpty();) this.nodeStack.push(t2), t2 = t2.right; } else { for (t2 = t2.right; !t2.isEmpty();) this.nodeStack.push(t2), t2 = t2.left; }
    return e;
  }
...
class je {
  ...
  remove(t2, e) {
    let n; let
      s = this;
    if (e(t2, s.key) < 0) { s.left.isEmpty() || s.left.isRed() || s.left.left.isRed() || (s = s.moveRedLeft()), s = s.copy(null, null, null, s.left.remove(t2, e), null); } else {
  ...
  colorFlip() {
    const t2 = this.left.copy(null, null, !this.left.color, null, null); const
  ...
  check() {
    ...
    if (t2 !== this.right.check()) { throw O(); }
...
class We {
  ...
  isEqual(t2) {
    ...
    const e = this.data.getIterator(); const
      n = t2.data.getIterator();
    for (; e.hasNext();) {
      const t3 = e.getNext().key; const
        s = n.getNext().key;
      if (this.comparator(t3, s) !== 0) { return false; }
    }
    return true;
  }
...
class Ye {
  ...
  field(t2) {
    ...
    {
      let e = this.value;
      for (let n = 0; n < t2.length - 1; ++n) { if (e = (e.mapValue.fields || {})[t2.get(n)], !de(e)) return null; }
...
function Xe(t2) {
  ...
  return Bt(t2.fields, (t3, n) => {
    ...
    if (de(n)) {
      ...
      if (t4.length === 0) { e.push(s); } else { for (const n2 of t4) e.push(s.child(n2)); }
    } else { e.push(s); }
  }), new Je(e);
}
/**
...
function sn(t2, e) {
  ...
  for (let n = 0; n < t2.orderBy.length; n++) { if (!Ke(t2.orderBy[n], e.orderBy[n])) return false; }
  if (t2.filters.length !== e.filters.length) { return false; }
  for (let n = 0; n < t2.filters.length; n++) { if (!Ce(t2.filters[n], e.filters[n])) return false; }
...
function mn(t2) {
  ...
  if (e.dt === null) {
    ...
    const t3 = _n(e); const
      n = dn(e);
    if (t3 !== null && n === null) { t3.isKeyField() || e.dt.push(new Ue(t3)), e.dt.push(new Ue(ut.keyField(), 'asc')); } else {
      let t4 = false;
...
function gn(t2) {
  const e = $(t2);
  if (!e._t) {
    if (e.limitType === 'F') e._t = en(e.path, e.collectionGroup, mn(e), e.filters, e.limit, e.startAt, e.endAt);
    else {
      const t3 = [];
      ...
      const n = e.endAt ? new Ie(e.endAt.position, e.endAt.inclusive) : null; const
...
function An(t2, e) {
  return e.isFoundDocument() && (function (t3, e2) {
    ...
    for (const n of mn(t3)) if (!n.field.isKeyField() && e2.data.field(n.field) === null) return false;
    return true;
  }(t2, e)) && (function (t3, e2) {
    ...
    }(t3.startAt, mn(t3), e2))) return false;
    if (t3.endAt && !(function (t4, e3, n) {
      const s = Te(t4, e3, n);
      return t4.inclusive ? s >= 0 : s > 0;
    }(t3.endAt, mn(t3), e2))) return false;
    return true;
  }(t2, e));
}
function Rn(t2) {
...
function Pn(t2, e, n) {
  const s = t2.field.isKeyField() ? ct.comparator(e.key, n.key) : (function (t3, e2, n2) {
    const s2 = e2.data.field(t3); const
      i = n2.data.field(t3);
    return s2 !== null && i !== null ? ie(s2, i) : O();
  }(t2.field, e, n));
  switch (t2.dir) {
    case 'asc':
      return s;
...
function Mn(t2, e) {
  const n = qn(e);
  for (const e2 of t2.elements) { n.some((t3) => ne(t3, e2)) || n.push(e2); }
  return {
    arrayValue: {
      values: n,
    },
...
function $n(t2, e) {
  let n = qn(e);
  for (const e2 of t2.elements) { n = n.filter((t3) => !ne(t3, e2)); }
  return {
    arrayValue: {
      values: n,
    },
...
function zn(t2, e) {
  if (!t2.hasLocalMutations || e && e.fields.length === 0) { return null; }
  if (e === null) { return t2.isNoDocument() ? new is(t2.key, Qn.none()) : new Zn(t2.key, t2.data, Qn.none()); }
  {
    const n = t2.data; const
      s = Ye.empty();
...
function Hn(t2, e, n) {
  t2 instanceof Zn ? (function (t3, e2, n2) {
    const s = t3.value.clone(); const
      i = ns(t3.fieldTransforms, e2, n2.transformResults);
    s.setAll(i), e2.convertToFoundDocument(n2.version, s).setHasCommittedMutations();
  }(t2, e, n)) : t2 instanceof ts ? (function (t3, e2, n2) {
    if (!jn(t3.precondition, e2)) return void e2.convertToUnknownDocument(n2.version);
    const s = ns(t3.fieldTransforms, e2, n2.transformResults); const
      i = e2.data;
...
function Jn(t2, e, n, s) {
  return t2 instanceof Zn ? (function (t3, e2, n2, s2) {
    if (!jn(t3.precondition, e2)) return n2;
    const i = t3.value.clone(); const
      r = ss(t3.fieldTransforms, s2, e2);
    ...
    if (!jn(t3.precondition, e2)) return n2;
    const i = ss(t3.fieldTransforms, s2, e2); const
      r = e2.data;
    if (r.setAll(es(t3)), r.setAll(i), e2.convertToFoundDocument(e2.version, r).setHasLocalMutations(), n2 === null) return null;
    return n2.unionWith(t3.fieldMask.fields).unionWith(t3.fieldTransforms.map((t4) => t4.field));
  }(t2, e, n, s)) : (function (t3, e2, n2) {
    if (jn(t3.precondition, e2)) return e2.convertToNoDocument(e2.version).setHasLocalMutations(), null;
    return n2;
  }(t2, e, n));
}
function Yn(t2, e) {
  ...
  for (const s of t2.fieldTransforms) {
    const t3 = e.data.field(s.field); const
      i = Nn(s.transform, t3 || null);
...
function ns(t2, e, n) {
  ...
  for (let i = 0; i < n.length; i++) {
    const r = t2[i]; const o = r.transform; const
...
function hs(t2) {
  if (void 0 === t2) { return x('GRPC error has no .code'), B.UNKNOWN; }
  switch (t2) {
    case us.OK:
      return B.OK;
    case us.CANCELLED:
...
class ls {
  ...
  get(t2) {
    const e = this.mapKeyFn(t2); const
      n = this.inner[e];
    if (void 0 !== n) {
  ...
  set(t2, e) {
    const n = this.mapKeyFn(t2); const
      s = this.inner[n];
  ...
  delete(t2) {
    ...
    for (let s = 0; s < n.length; s++) { if (this.equalsFn(n[s][0], t2)) return n.length === 1 ? delete this.inner[e] : n.splice(s, 1), this.innerSize--, true; }
    return false;
  }
...
class Cs {
  ...
  Jt(t2) {
    const e = t2.targetId; const n = t2.Et.count; const
      s = this.Yt(e);
    if (s) {
      const t3 = s.target;
      if (rn(t3)) {
...
function Qs(t2, e) {
  const n = Ks(e);
  if (n.get(1) !== t2.databaseId.projectId) { throw new L(B.INVALID_ARGUMENT, `Tried to deserialize key from different project: ${n.get(1)} vs ${t2.databaseId.projectId}`); }
  if (n.get(3) !== t2.databaseId.database) { throw new L(B.INVALID_ARGUMENT, `Tried to deserialize key from different database: ${n.get(3)} vs ${t2.databaseId.database}`); }
  return new ct(Hs(n));
}
function js(t2, e) {
...
function Hs(t2) {
  return M(t2.length > 4 && t2.get(4) === 'documents'), t2.popFirst(5);
}
function Js(t2, e, n) {
  return {
    name: Gs(t2, e),
...
function Zs(t2, e) {
  ...
  if ('targetChange' in e) {
    e.targetChange;
    const s = (function (t3) {
      return t3 === 'NO_CHANGE' ? 0 : t3 === 'ADD' ? 1 : t3 === 'REMOVE' ? 2 : t3 === 'CURRENT' ? 3 : t3 === 'RESET' ? 4 : O();
    }(e.targetChange.targetChangeType || 'NO_CHANGE')); const i = e.targetChange.targetIds || []; const r = (function (t3, e2) {
      return t3.wt ? (M(void 0 === e2 || typeof e2 === 'string'), Qt.fromBase64String(e2 || '')) : (M(void 0 === e2 || e2 instanceof Uint8Array), Qt.fromUint8Array(e2 || new Uint8Array()));
    }(t2, e.targetChange.resumeToken)); const o = e.targetChange.cause; const
      u = o && (function (t3) {
    ...
    const i = Qs(t2, s.document.name); const r = qs(s.document.updateTime); const o = s.document.createTime ? qs(s.document.createTime) : st.min(); const u = new Ye({
      mapValue: {
        fields: s.document.fields,
      },
    }); const c = Ze.newFoundDocument(i, r, o, u); const a = s.targetIds || []; const
    ...
    s.document;
    const i = Qs(t2, s.document); const r = s.readTime ? qs(s.readTime) : st.min(); const o = Ze.newNoDocument(i, r); const
    ...
    s.document;
    const i = Qs(t2, s.document); const
    ...
    if (!('filter' in e)) { return O(); }
    {
      ...
      const t3 = e.filter;
      t3.targetId;
      const s = t3.count || 0; const i = new os(s); const
        r = t3.targetId;
      n = new Vs(r, i);
    }
  }
...
function ri(t2) {
  let e = Ws(t2.parent);
  const n = t2.structuredQuery; const
  ...
  n.where && (r = (function (t3) {
    const e2 = ui(t3);
    if (e2 instanceof be && Ve(e2)) return e2.getFilters();
    return [e2];
  }(n.where)));
  let o = [];
...
class Qi {
  ...
  applyToLocalView(t2, e) {
    for (const n of this.baseMutations) { n.key.isEqual(t2.key) && (e = Jn(n, t2, e, this.localWriteTime)); }
    for (const n of this.mutations) { n.key.isEqual(t2.key) && (e = Jn(n, t2, e, this.localWriteTime)); }
    return e;
  }
...
class kr {
  ...
  add(t2) {
    const e = t2.lastSegment(); const n = t2.popLast(); const s = this.index[e] || new We(rt.comparator); const
      i = !s.has(n);
    return this.index[e] = s.add(n), i;
  }

  has(t2) {
    const e = t2.lastSegment(); const n = t2.popLast(); const
      s = this.index[e];
    return s && s.has(n);
  }
...
class po {
  ...
  computeViews(t2, e, n, s) {
    ...
    return e.forEach((t3, e2) => {
      ...
      let n2;
      return o.set(t4, new yo(e2, (n2 = r.get(t4)) !== null && void 0 !== n2 ? n2 : null));
    }), o));
  }

  recalculateAndSaveOverlays(t2, e) {
    const n = ps();
    let s = new Ge((t3, e2) => t3 - e2); let
      i = Es();
    return this.mutationQueue.getAllMutationBatchesAffectingDocumentKeys(t2, e).next((t3) => {
      ...
      for (; o.hasNext();) {
        const s2 = o.getNext(); const u = s2.key; const c = s2.value; const
          a = ys();
        c.forEach((t3) => {
          if (!i.has(t3)) {
  ...
  getDocumentsMatchingCollectionQuery(t2, e, n) {
    ...
    return this.remoteDocumentCache.getAllFromCollection(t2, e.path, n).next((i) => (s = i, this.documentOverlayCache.getOverlaysForCollection(t2, e.path, n.largestBatchId))).next((t3) => {
      t3.forEach((t4, e2) => {
        const n3 = e2.getKey();
        s.get(n3) === null && (s = s.insert(n3, Ze.newInvalidDocument(n3)));
      });
      let n2 = ws();
...
class To {
  ...
  getOverlaysForCollection(t2, e, n) {
    const s = gs(); const i = e.length + 1; const r = new ct(e.child('')); const
      o = this.overlays.getIteratorFrom(r);
    for (; o.hasNext();) {
      const t3 = o.getNext().value; const
        r2 = t3.getKey();
...
class Eo {
  ...
  ls(t2) {
    const e = new ct(new rt([])); const n = new Ao(e, t2); const s = new Ao(e, t2 + 1); const
      i = [];
  ...
  ds(t2) {
    const e = new ct(new rt([])); const n = new Ao(e, t2); const
      s = new Ao(e, t2 + 1);
    let i = Es();
    return this.rs.forEachInRange([n, s], (t3) => {
      i = i.add(t3.key);
...
class Ro {
  ...
  addMutationBatch(t2, e, n, s) {
    ...
    for (const e2 of s) { this.gs = this.gs.add(new Ao(e2.key, i)), this.indexManager.addToCollectionParentIndex(t2, e2.key.path.popLast()); }
    return At.resolve(r);
  }
  ...
  getNextMutationBatchAfterBatchId(t2, e) {
    const n = e + 1; const s = this.ps(n); const
      i = s < 0 ? 0 : s;
  ...
  getAllMutationBatchesAffectingDocumentKey(t2, e) {
    const n = new Ao(e, 0); const s = new Ao(e, Number.POSITIVE_INFINITY); const
      i = [];
  ...
  getAllMutationBatchesAffectingDocumentKeys(t2, e) {
    let n = new We(Z);
    return e.forEach((t3) => {
      const e2 = new Ao(t3, 0); const
        s = new Ao(t3, Number.POSITIVE_INFINITY);
  ...
  removeMutationBatch(t2, e) {
    M(this.Ts(e.batchId, 'removed') === 0), this.mutationQueue.shift();
    let n = this.gs;
...
class bo {
  ...
  addEntry(t2, e) {
    const n = e.key; const s = this.docs.get(n); const i = s ? s.size : 0; const
      r = this.Es(e);
...
class vo {
  ...
  removeMatchingKeys(t2, e, n) {
    this.Ps.hs(e, n);
    const s = this.persistence.referenceDelegate; const
      i = [];
...
class Bo {
  ...
  ki(t2, e) {
    if (fn(e)) { return At.resolve(null); }
    let n = gn(e);
...
function Ko(t2, e) {
  ...
  return n.persistence.runTransaction('Acknowledge batch', 'readwrite-primary', (t3) => {
    const s = e.batch.keys(); const
      i = n.Gi.newChangeBuffer({
        trackRemovals: true,
      });
    return (function (t4, e2, n2, s2) {
      const i2 = n2.batch; const
        r = i2.keys();
      let o = At.resolve();
      return r.forEach((t5) => {
        o = o.next(() => s2.getEntry(e2, t5)).next((e3) => {
          const r2 = n2.docVersions.get(t5);
          M(r2 !== null), e3.version.compareTo(r2) < 0 && (i2.applyToRemoteDocument(e3, n2), e3.isValidDocument() && (e3.setReadTime(n2.commitVersion), s2.addEntry(e3)));
        });
      }), o.next(() => t4.mutationQueue.removeMutationBatch(e2, i2));
    }(n, t3, e, i)).next(() => i.apply(t3)).next(() => n.mutationQueue.performConsistencyCheck(t3)).next(() => n.documentOverlayCache.removeOverlaysForBatchId(t3, s, e.batch.batchId))
      .next(() => n.localDocuments.recalculateAndSaveOverlaysForDocumentKeys(t3, (function (t4) {
        let e2 = Es();
      ...
      .next(() => n.localDocuments.getDocuments(t3, s));
  });
}
function Go(t2) {
  const e = $(t2);
  ...
  return n.persistence.runTransaction('Apply remote event', 'readwrite-primary', (t3) => {
    ...
    e.targetChanges.forEach((r2, u2) => {
      ...
      e.targetMismatches.has(u2) ? a = a.withResumeToken(Qt.EMPTY_BYTE_STRING, st.min()).withLastLimboFreeSnapshotVersion(st.min()) : r2.resumeToken.approximateByteSize() > 0 && (a = a.withResumeToken(r2.resumeToken, s)), i = i.insert(u2, a), (function (t4, e2, n2) {
        if (t4.resumeToken.approximateByteSize() === 0) return true;
...
async function Ho(t2, e, n) {
  const s = $(t2); const i = s.qi.get(e); const
...
class du {
  ...
  Gr() {
    ...
    for (const t2 of this.Wr) { t2(0); }
  }

  jr() {
    C('ConnectivityMonitor', 'Network connectivity changed: UNAVAILABLE');
    for (const t2 of this.Wr) { t2(1); }
  }

  static C() {
    return typeof window !== 'undefined' && void 0 !== window.addEventListener && void 0 !== window.removeEventListener;
  }
}
/**
 * @license
...
class mu extends class {
        ...
        try {
          switch (o.getLastErrorCode()) {
            ...
            case ErrorCode.HTTP_ERROR:
              ...
              } else { r(new L(B.UNAVAILABLE, 'Connection failed.')); }
              break;
            default:
              O();
          }
  ...
  wo(t2, e, n) {
    const s = [this.oo, '/', 'google.firestore.v1.Firestore', '/', t2, '/channel']; const i = createWebChannelTransport(); const r = getStatEventTarget(); const
      o = {
...
class Iu {
  ...
  Ro(t2) {
    this.cancel();
    const e = Math.floor(this.Io + this.bo()); const n = Math.max(0, Date.now() - this.Eo); const
      s = Math.max(0, e - n);
...
class Tu {
  ...
  async close(t2, e) {
    this.Lo(), this.qo(), this.xo.cancel(), this.So++, t2 !== 4 ? this.xo.reset() : e && e.code === B.RESOURCE_EXHAUSTED ? (x(e.toString()), x('Using maximum backoff delay to prevent overloading the backend.'), this.xo.Ao()) : e && e.code === B.UNAUTHENTICATED && this.state !== 3 && (this.authCredentialsProvider.invalidateToken(), this.appCheckCredentialsProvider.invalidateToken()), this.stream !== null && (this.Uo(), this.stream.close(), this.stream = null), this.state = t2, await this.listener.Zr(e);
  }
  ...
  auth() {
    this.state = 1;
    const t2 = this.Ko(this.So); const
      e = this.So;
...
class Au extends Tu {
  ...
  onMessage(t2) {
    if (M(!!t2.streamToken), this.lastStreamToken = t2.streamToken, this.Jo) {
      this.xo.reset();
      const e = ni(t2.writeResults, t2.commitTime); const
        n = qs(t2.commitTime);
      return this.listener.Zo(n, e);
    }
    return M(!t2.writeResults || t2.writeResults.length === 0), this.Jo = true, this.listener.tu();
  }
...
class Ru extends class {
  ...
  ao(t2, e, n) {
    return this.su(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([s, i]) => this.connection.ao(t2, e, n, s, i)).catch((t3) => {
      throw t3.name === 'FirebaseError' ? (t3.code === B.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), t3) : new L(B.UNKNOWN, t3.toString());
    });
  }
...
async function Vu(t2) {
  if (Mu(t2)) { for (const e of t2.wu) await e(true); }
}
async function Su(t2) {
  for (const e of t2.wu) { await e(false); }
}
function Du(t2, e) {
  const n = $(t2);
  ...
  if (t2.gu.set('Online'), e instanceof Ss && e.state === 2 && e.cause) {
    try {
      await (async function (t3, e2) {
        const n2 = e2.cause;
        for (const s of e2.targetIds) t3.du.has(s) && (await t3.remoteSyncer.rejectListen(s, n2), t3.du.delete(s), t3.yu.removeTarget(s));
      }(t2, e));
    } catch (n2) {
...
class nc {
  ...
  cancel(t2) {
    this.timerHandle !== null && (this.clearTimeout(), this.deferred.reject(new L(B.CANCELLED, `Operation cancelled${t2 ? `: ${t2}` : ''}`)));
  }
...
function sc(t2, e) {
  if (x('AsyncQueue', `${e}: ${t2}`), Vt(t2)) { return new L(B.UNAVAILABLE, `${e}: ${t2}`); }
  throw t2;
}
/**
 * @license
...
class ic {
  ...
  isEqual(t2) {
    if (!(t2 instanceof ic)) { return false; }
    if (this.size !== t2.size) { return false; }
    const e = this.sortedSet.getIterator(); const
...
class oc {
  ...
  isEqual(t2) {
    if (!(this.fromCache === t2.fromCache && this.hasCachedResults === t2.hasCachedResults && this.syncStateChanged === t2.syncStateChanged && this.mutatedKeys.isEqual(t2.mutatedKeys) && In(this.query, t2.query) && this.docs.isEqual(t2.docs) && this.oldDocs.isEqual(t2.oldDocs))) { return false; }
    const e = this.docChanges; const
      n = t2.docChanges;
...
class _c {
  ...
  Pu(t2) {
    if (!this.options.includeMetadataChanges) {
      const e2 = [];
      for (const n of t2.docChanges) n.type !== 3 && e2.push(n);
      t2 = new oc(
        t2.query,
        t2.docs,
        t2.oldDocs,
...
class Tc {
  ...
  Wu(t2, e) {
    const n = e ? e.zu : new rc(); const
      s = e ? e.Qu : this.Qu;
    let i = e ? e.mutatedKeys : this.mutatedKeys; let r = s; let
      o = false;
    const u = this.query.limitType === 'F' && s.size === this.query.limit ? s.last() : null; const
      c = this.query.limitType === 'L' && s.size === this.query.limit ? s.first() : null;
    if (t2.inorderTraversal((t3, e2) => {
      const a = s.get(t3); const h = An(this.query, e2) ? e2 : null; const l2 = !!a && this.mutatedKeys.has(a.key); const
        f = !!h && (h.hasLocalMutations || this.mutatedKeys.has(h.key) && h.hasCommittedMutations);
      let d = false;
      if (a && h) {
  ...
  applyChanges(t2, e, n) {
    ...
    const r = e ? this.Yu() : []; const o = this.Ku.size === 0 && this.current ? 1 : 0; const
      u = o !== this.Uu;
  ...
  ec() {
    return oc.fromInitialDocuments(this.query, this.Qu, this.mutatedKeys, this.Uu === 0, this.hasCachedResults);
  }
}
class Ec {
  constructor(t2, e, n) {
...
async function bc(t2, e) {
  ...
  let s; let
    i;
  const r = n.ic.get(e);
  if (r) { s = r.targetId, n.sharedClientState.addLocalQueryTarget(s), i = r.view.ec(); } else {
    ...
    s = t3.targetId, i = await Pc(n, e, s, r2 === 'current', t3.resumeToken);
  }
  return i;
}
async function Pc(t2, e, n, s, i) {
  t2._c = (e2, n2, s2) => (async function (t3, e3, n3, s3) {
    ...
    const r2 = s3 && s3.targetChanges.get(e3.targetId); const
      o2 = e3.view.applyChanges(
        i2,
        t3.isPrimaryClient,
        r2,
      );
  ...
  const r = await Jo(
    ...
    e,
    true,
  ); const o = new Tc(e, r.Hi); const u = o.Wu(r.documents); const c = Ps.createSynthesizedTargetChangeForCurrentChange(n, s && t2.onlineState !== 'Offline', i); const
    a = o.applyChanges(
      u,
      t2.isPrimaryClient,
      c,
...
async function vc(t2, e) {
  const n = $(t2); const s = n.ic.get(e); const
    i = n.rc.get(s.targetId);
  if (i.length > 1) { return n.rc.set(s.targetId, i.filter((t3) => !In(t3, e))), void n.ic.delete(e); }
...
async function Vc(t2, e, n) {
  ...
  try {
    const t3 = await (function (t4, e2) {
      const n2 = $(t4); const s2 = nt.now(); const
        i = e2.reduce((t5, e3) => t5.add(e3.key), Es());
      let r; let
        o;
  ...
  if (s.isPrimaryClient && n === 0 || !s.isPrimaryClient && n === 1) {
    ...
    s.ic.forEach((n2, s2) => {
      ...
      n2.queries.forEach((t5, n3) => {
        for (const t6 of n3.listeners) t6.bu(e2) && (s2 = true);
      }), s2 && dc(n2);
  ...
  try {
    const t3 = await (function (t4, e2) {
      ...
      return n2.persistence.runTransaction('Reject batch', 'readwrite-primary', (t5) => {
        let s2;
        return n2.mutationQueue.lookupMutationBatch(t5, e2).next((e3) => (M(e3 !== null), s2 = e3.keys(), n2.mutationQueue.removeMutationBatch(t5, e3))).next(() => n2.mutationQueue.performConsistencyCheck(t5)).next(() => n2.documentOverlayCache.removeOverlaysForBatchId(t5, s2, e2))
          .next(() => n2.localDocuments.recalculateAndSaveOverlaysForDocumentKeys(t5, s2))
          .next(() => n2.localDocuments.getDocuments(t5, s2));
      });
    }(s.localStore, e));
...
function $c(t2, e) {
  ...
  const n = t2.uc.get(e);
  n !== null && (Cu(t2.remoteStore, n), t2.uc = t2.uc.remove(e), t2.cc.delete(n), qc(t2));
}
function Bc(t2, e, n) {
  for (const s of n) {
    if (s instanceof pc) t2.ac.addReference(s.key, e), Lc(t2, s);
...
async function Uc(t2, e, n) {
  ...
  s.ic.isEmpty() || (s.ic.forEach((t3, u) => {
    ...
    for (const t4 of e2) {
      ...
      if (!t4.fromCache) {
        const t5 = n2.qi.get(e3); const s2 = t5.snapshotVersion; const
          i2 = t5.withLastLimboFreeSnapshotVersion(s2);
...
class oa {
  ...
  createDatastore(t2) {
    const e = pu(t2.databaseInfo.databaseId); const
      n = (s = t2.databaseInfo, new mu(s));
    let s;
 ...
 */
function ua(t2, e, n) {
  if (!n) { throw new L(B.INVALID_ARGUMENT, `Function ${t2}() cannot be called with an empty ${e}.`); }
}
function ca(t2, e, n, s) {
  if (e === true && s === true) { throw new L(B.INVALID_ARGUMENT, `${t2} and ${n} cannot be used together.`); }
}
function aa(t2) {
  if (!ct.isDocumentKey(t2)) { throw new L(B.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${t2} has ${t2.length}.`); }
}
function ha(t2) {
  if (ct.isDocumentKey(t2)) { throw new L(B.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${t2} has ${t2.length}.`); }
}
function la(t2) {
  if (void 0 === t2) { return 'undefined'; }
  if (t2 === null) { return 'null'; }
  if (typeof t2 === 'string') { return t2.length > 20 && (t2 = `${t2.substring(0, 20)}...`), JSON.stringify(t2); }
  if (typeof t2 === 'number' || typeof t2 === 'boolean') { return `${t2}`; }
  if (typeof t2 === 'object') {
  ...
  return typeof t2 === 'function' ? 'a function' : O();
}
function fa(t2, e) {
  if ('_delegate' in t2 && (t2 = t2._delegate), !(t2 instanceof e)) {
    if (e.name === t2.constructor.name) { throw new L(B.INVALID_ARGUMENT, 'Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?'); }
    {
      const n = la(t2);
...
class wa {
  constructor(t2) {
    let e;
    if (void 0 === t2.host) {
      if (void 0 !== t2.ssl) { throw new L(B.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set"); }
      this.host = 'firestore.googleapis.com', this.ssl = true;
    } else { this.host = t2.host, this.ssl = (e = t2.ssl) === null || void 0 === e || e; }
    if (this.credentials = t2.credentials, this.ignoreUndefinedProperties = !!t2.ignoreUndefinedProperties, void 0 === t2.cacheSizeBytes) { this.cacheSizeBytes = 41943040; } else {
      if (t2.cacheSizeBytes !== -1 && t2.cacheSizeBytes < 1048576) { throw new L(B.INVALID_ARGUMENT, 'cacheSizeBytes must be at least 1048576'); }
...
function ga(t2, e, n, s = {}) {
  ...
  if (r.host !== 'firestore.googleapis.com' && r.host !== e && N('Host has been set in both settings() and useEmulator(), emulator host will be used'), t2._setSettings({
    ...r,
    host: `${e}:${n}`,
    ssl: false,
  }), s.mockUserToken) {
    let e2; let
      n2;
    if (typeof s.mockUserToken === 'string') { e2 = s.mockUserToken, n2 = P.MOCK_USER; } else {
      e2 = createMockUserToken(s.mockUserToken, (i = t2._app) === null || void 0 === i ? void 0 : i.options.projectId);
      const r2 = s.mockUserToken.sub || s.mockUserToken.user_id;
      if (!r2) { throw new L(B.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!"); }
      n2 = new P(r2);
    }
...
function Ta(t2, e, ...n) {
  ...
  {
    if (!(t2 instanceof ya || t2 instanceof Ia)) { throw new L(B.INVALID_ARGUMENT, 'Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore'); }
...
function Aa(t2, e, ...n) {
  if (t2 = getModularInstance(t2), arguments.length === 1 && (e = X.R()), ua('doc', 'path', e), t2 instanceof ma) {
    const s = rt.fromString(e, ...n);
    return aa(s), new ya(
      t2,
      null,
...
class th {
  ...
  Zc(t2) {
    for (const e of this.Uc) { if (e.timerId === t2) return true; }
    return false;
  }
  ...
  ta(t2) {
    return this.Xc().then(() => {
      ...
      for (const e of this.Uc) { if (e.skipDelay(), t2 !== 'all' && e.timerId === t2) break; }
      return this.Xc();
    });
  }
...
function eh(t2) {
  return (function (t3, e) {
    ...
    const n = t3;
    for (const t4 of e) if (t4 in n && typeof n[t4] === 'function') return true;
    return false;
...
function oh(e, n) {
  const s = typeof e === 'object' ? e : getApp(); const i = typeof e === 'string' ? e : n || '(default)'; const
...
function ch(t2) {
  let e;
  const n = t2._freezeSettings(); const
    s = (function (t3, e2, n2, s2) {
    ...
    }(t2._databaseId, ((e = t2._app) === null || void 0 === e ? void 0 : e.options.appId) || '', t2._persistenceKey, n));
...
class Th {
  constructor(...t2) {
    for (let e = 0; e < t2.length; ++e) { if (t2[e].length === 0) throw new L(B.INVALID_ARGUMENT, 'Invalid field name at argument $(i + 1). Field names must not be empty.'); }
...
class Rh {
  constructor(t2, e) {
    if (!isFinite(t2) || t2 < -90 || t2 > 90) { throw new L(B.INVALID_ARGUMENT, `Latitude must be a number between -90 and 90, but was: ${t2}`); }
...
class Ph {
  ...
  toMutation(t2, e) {
    return this.fieldMask !== null ? new ts(t2, this.data, this.fieldMask, e, this.fieldTransforms) : new Zn(t2, this.data, e, this.fieldTransforms);
  }
}
class vh {
  constructor(t2, e, n) {
...
class Sh {
  ...
  ia(t2) {
    return new Sh(({ ...this.settings, ...t2 }), this.databaseId, this.yt, this.ignoreUndefinedProperties, this.fieldTransforms, this.fieldMask);
  }

  ra(t2) {
    let e;
    const n = (e = this.path) === null || void 0 === e ? void 0 : e.child(t2); const
      s = this.ia({
        path: n,
        oa: false,
      });
  ...
  ua(t2) {
    if (t2.length === 0) { throw this.ha('Document fields must not be empty'); }
    if (Vh(this.sa) && bh.test(t2)) { throw this.ha('Document fields cannot begin and end with "__"'); }
  }
}
class Dh {
  constructor(t2, e, n) {
...
function Ch(t2) {
  const e = t2._freezeSettings(); const
    n = pu(t2._databaseId);
...
function xh(t2, e, n, s, i, r = {}) {
  ...
  if (r.merge) { c = new Je(o.fieldMask), a = o.fieldTransforms; } else if (r.mergeFields) {
    ...
    for (const s2 of r.mergeFields) {
      const i2 = jh(e, s2, n);
      if (!o.contains(i2)) { throw new L(B.INVALID_ARGUMENT, `Field '${i2}' is specified in your field mask but missing from your input data.`); }
      Jh(t3, i2) || t3.push(i2);
    }
...
class Nh extends Ah {
  _toFieldTransform(t2) {
    if (t2.sa !== 2) { throw t2.sa === 1 ? t2.ha(`${this._methodName}() can only appear at the top level of your update data`) : t2.ha(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`); }
...
function Bh(t2, e, n, s) {
  ...
  Bt(s, (t3, s2) => {
    ...
    const c = i.ca(u2);
    if (s2 instanceof Nh) { r.push(u2); } else {
      const t4 = Uh(s2, c);
...
function Lh(t2, e, n, s, i, r) {
  const o = t2.da(1, e, n); const u = [jh(e, s, n)]; const
    c = [i];
  ...
  for (let t3 = 0; t3 < r.length; t3 += 2) { u.push(jh(e, r[t3])), c.push(r[t3 + 1]); }
  const a = []; const
    h = Ye.empty();
...
function Uh(t2, e) {
  ...
  if (t2 instanceof Ah) {
    return (function (t3, e2) {
      if (!Vh(e2.sa)) throw e2.ha(`${t3._methodName}() can only be used with update() and set()`);
      if (!e2.path) throw e2.ha(`${t3._methodName}() is not currently supported inside arrays`);
      const n = t3._toFieldTransform(e2);
      n && e2.fieldTransforms.push(n);
    }(t2, e)), null;
  }
  if (void 0 === t2 && e.ignoreUndefinedProperties) { return null; }
  ...
  return (function (t3, e2) {
    ...
    if (typeof t3 === 'number') return Sn(e2.yt, t3);
    ...
    if (t3 instanceof ya) {
      const n = e2.databaseId; const
        s = t3.firestore._databaseId;
...
function Gh(t2) {
  return !(typeof t2 !== 'object' || t2 === null || t2 instanceof Array || t2 instanceof Date || t2 instanceof nt || t2 instanceof Rh || t2 instanceof Ih || t2 instanceof ya || t2 instanceof Ah);
}
function Qh(t2, e, n) {
  if (!Gh(n) || !(function (t3) {
...
function jh(t2, e, n) {
  if ((e = getModularInstance(e)) instanceof Th) { return e._internalPath; }
  if (typeof e === 'string') { return zh(t2, e); }
  throw Hh(
 ...
 */
function tl(t2) {
  if (t2.limitType === 'L' && t2.explicitOrderBy.length === 0) { throw new L(B.UNIMPLEMENTED, 'limitToLast() queries require specifying at least one orderBy() clause'); }
}
class Pl {
  ...
  convertDocumentKey(t2, e) {
    ...
    M(wi(n));
    const s = new Ft(n.get(1), n.get(3)); const
...
function Kl(t2, e, n, ...s) {
  t2 = fa(t2, ya);
  const i = fa(t2.firestore, ih); const
    r = Ch(i);
  let o;
...
function Ql(t2, e) {
  const n = fa(t2.firestore, ih); const s = Aa(t2); const
    i = vl(t2.converter, e);
  return zl(n, [xh(Ch(t2.firestore), 'addDoc', s._key, i, t2.converter !== null, {}).toMutation(s._key, Qn.exists(false))]).then(() => s);
}
function jl(t2, ...e) {
  let n; let s; let
    i;
  ...
  if (eh(e[o])) {
    const t3 = e[o];
    e[o] = (n = t3.next) === null || void 0 === n ? void 0 : n.bind(t3), e[o + 1] = (s = t3.error) === null || void 0 === s ? void 0 : s.bind(t3), e[o + 2] = (i = t3.complete) === null || void 0 === i ? void 0 : i.bind(t3);
  }
  let c; let a; let
    h;
  if (t2 instanceof ya) {
...
function Hl(t2, e, n) {
  const s = n.docs.get(e._key); const
    i = new Ml(t2);
...
!(function (t2, e = true) {
  !(function (t3) {
    ...
    const i = t3.getProvider('app').getImmediate(); const
      r = new ih(new Q(t3.getProvider('auth-internal')), new H(t3.getProvider('app-check-internal')), (function (t4, e2) {
        if (!Object.prototype.hasOwnProperty.apply(t4.options, ['projectId'])) throw new L(B.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
        return new Ft(t4.options.projectId, e2);
      }(i, n)), i);
    return s = { useFetchStreams: e, ...s }, r._setSettings(s), r;
...
const updateData = (userName) => updateProfile(auth.currentUser, { displayName: userName });
...
const sendPassword = (email) => sendPasswordResetEmail(auth, email);
const saveTask = (COMMENT) => Ql(Ta(db, 'tasks'), COMMENT);
const getAllTasks = (querySnapshot) => {
  jl(Ta(db, 'tasks'), querySnapshot);
};
...
const register = (email, password, userName, cellPhone) => new Promise((resolve, reject) => {
  signUp(email, password).then((userCredential) => {
    updateData(userName).then((resolves) => {
      sendEmail().then(() => {
        Swal.fire({
...
const startGoogle = () => new Promise((resolve, reject) => {
...
const startSignIn = (email, password) => new Promise((resolve, reject) => {
...
const creatingPost = (title, description) => saveTask(title);
const start = () => {
  const viewStart = `
...
const login = () => {
  ...
  buttonIngresar.addEventListener('click', (e) => {
    e.preventDefault();
    startSignIn(inputEmail.value, inputPassword.value).then((success) => {
      console.log(success);
      console.log('Aqui Cambio de vista Ok');
      window.location.hash = '#/home';
    }).catch((error) => {
...
const different = () => {
  ...
  divElement.innerHTML = different2;
  return divElement;
};
const RegisterForm = () => {
  const viewLoginEm = `
...
const deletePost = (tasksContainer) => {
        ...
        btn.addEventListener('click', () => {
          ...
          `;
            ...
            buttonEdit.addEventListener('click', () => {
              ...
              updateTask(id2, newTitle);
              updateTask(id2, newPost);
            });
          });
        });
...
const changePage = (hash) => {
  ...
  switch (hash) {
    ...
    case '#':
    case '#/': {
      return containerBody.appendChild(components.Start());
    }
    case '#/login': {
      return containerBody.appendChild(components.Login());
    }
    case '#/email': {
      return containerBody.appendChild(components.Email());
    }
    case '#/home': {
      return containerBody.appendChild(components.Home());
    }
    default:
